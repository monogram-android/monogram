...
internal class StoredMtProtoAuthKey private constructor(
    ...
    companion object {
        ...
        fun create(material: ByteArray, id: Long, serverSalt: Long, createdAt: Int): StoredMtProtoAuthKey {
            ...
            return StoredMtProtoAuthKey(material, id, serverSalt, createdAt)
        }
...
internal object MtProtoAuthKeyRecordCodec {
    ...
    private const val ENCODED_BYTES = 4 + 4 + 8 + 8 + 4 + 4 + StoredMtProtoAuthKey.MATERIAL_BYTES
    ...
    fun decode(bytes: ByteArray): StoredMtProtoAuthKey {
        require(bytes.size == ENCODED_BYTES) { "Invalid MTProto auth key record length" }
        val buffer = ByteBuffer.wrap(bytes).order(ByteOrder.BIG_ENDIAN)
        require(buffer.int == MAGIC) { "Invalid MTProto auth key record magic" }
        require(buffer.int == VERSION) { "Unsupported MTProto auth key record version" }
        val id = buffer.long
        val serverSalt = buffer.long
        val createdAt = buffer.int
        ...
        buffer.get(material)
        return try {
            StoredMtProtoAuthKey.create(material, id, serverSalt, createdAt)
        } finally {
            material.fill(0)
        }
    }
