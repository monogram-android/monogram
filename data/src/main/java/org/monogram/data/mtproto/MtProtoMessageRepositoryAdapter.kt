...
/**
 * Keeps Telegram-owned message commands unavailable when the account uses the Kotlin MTProto
 * backend. The snapshot repositories own the currently supported MTProto read path instead.
 */
internal class MtProtoMessageRepositoryAdapter(
        ...
        override fun invoke(proxy: Any, method: Method, args: Array<out Any?>?): Any? {
            if (method.declaringClass == Any::class.java) {
                return when (method.name) {
                    ...
                    "toString" -> "MtProtoMessageRepositoryAdapter"
