//! Rebuild Telegram `photoStrippedSize` (`i`) into a JPEG.
...
/// `bytes[0] == 1` is the compact form: header + payload[3..] + JPEG EOI.
pub(crate) fn expand_stripped_jpeg(bytes: &[u8]) -> Option<Vec<u8>> {
    ...
    let mut out = Vec::with_capacity(HEADER.len() + bytes.len() - 3 + FOOTER.len());
