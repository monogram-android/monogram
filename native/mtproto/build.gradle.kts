...
val buildNativeMtproto =
    tasks.register("buildNativeMtproto") {
        group = "build"
        description =
...
val buildHostUniffiMtproto =
    tasks.register<Exec>("buildHostUniffiMtproto") {
        group = "build"
        description =
...
val generateUniffiMtproto =
    tasks.register<Exec>("generateUniffiMtproto") {
        group = "build"
        description =
            "Regenerate UniFFI Kotlin bindings from the host debug library (skip with -PskipNativeBuild=true)"
        dependsOn(buildHostUniffiMtproto)
        workingDir = rustCrate.asFile
        environment("PATH", cargoPath)
        commandLine(
            cargoCommand(
...
val cleanOrphanedLibraries =
    tasks.register<Delete>("cleanOrphanedLibraries") {
        delete(fileTree(jniLibs) {
            include("**/libtlottie.so")
        })
    }

buildNativeMtproto.configure {
    finalizedBy(cleanOrphanedLibraries, generateUniffiMtproto)
}

tasks.named("preBuild").configure {
    dependsOn(buildNativeMtproto, generateUniffiMtproto)
}
