...
class DateUtilsTest {
    ...
    fun `When date are today, then returns only hours and minutes`() {
        ...
        val result = targetDate.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday)
    ...
    fun `When date are yesterday and within 1 week, then returns day of week`() {
        ...
        assertEquals("вт", yesterday.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))

        // Six days ago
        val sixDaysAgo = dateFormatter.parse("14.03.2024 09:00")!!
        assertEquals("чт", sixDaysAgo.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))
    }
    ...
    fun `When date are older than 1 week but within 1 year, then return day and month`() {
        ...
        assertEquals("13 мар", sevenDaysAgo.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))

        // Прошлый месяц (меньше 365 дней назад)
        val monthsAgo = dateFormatter.parse("25.10.2023 18:30")!!
        assertEquals("25 окт", monthsAgo.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))
    }
    ...
    fun `When date are older than 1 year, then return full date`() {
        ...
        assertEquals("10.03.2023", olderThanYear.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))

        // A long time ago...
        val veryOldDate = dateFormatter.parse("01.01.2020 00:00")!!
        assertEquals("01.01.2020", veryOldDate.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))
    }

    @Test
    fun `Then future date is older than 1 year, then return full date`() {
        val futureDate = dateFormatter.parse("16.03.2029 10:00")!!
        assertEquals("16.03.2029", futureDate.toShortRelativeDate(timeFormat = "HH:mm", locale = ruLocale, now = mockToday))
    }
}
